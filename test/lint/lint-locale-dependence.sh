...
# Be aware that xcbcd and xcbc-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in xcbcd.
#
...
# C locale.
#
# In contrast, xcbcd does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where xcbcd is guaranteed to be running
# with the classic locale ("C") whereas the locale of xcbc-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in xcbcd but
# "1,230000" in xcbc-qt.
#
...
KNOWN_VIOLATIONS=(
    "src/xcbc-tx.cpp.*stoul"
    "src/xcbc-tx.cpp.*trim_right"
