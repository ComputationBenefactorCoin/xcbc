...
// For details see https://github.com/ComputationBenefactorCoin/xcbc/pull/22348.
...
const char * const XCBC_CONF_FILENAME = "xcbc.conf";
const char * const XCBC_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/ComputationBenefactorCoin/xcbc/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //xcbc-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", XCBC_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\XCBC
    // macOS: ~/Library/Application Support/XCBC
    // Unix-like: ~/.xcbc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XCBC";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/XCBC";
#else
    // Unix-like
    return pathRet / ".xcbc";
#endif
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", XCBC_CONF_FILENAME);
