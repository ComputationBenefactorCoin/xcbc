...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<XCBCAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_xcbc-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_xcbc-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_xcbc-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = XCBCUnits::formatWithUnit(unit, balance, false, XCBCUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    XCBCAmountField* amountInput = receiveCoinsDialog.findChild<XCBCAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_xcbc-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
