...
#include <qt/xcbcaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a XCBC address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new XCBCAddressEntryValidator(parent));
    widget->setCheckValidator(new XCBCAddressCheckValidator(parent));
}
...
bool parseXCBCURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no xcbc: URI
    if(!uri.isValid() || uri.scheme() != QString("xcbc"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!XCBCUnits::parse(XCBCUnits::XCBC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseXCBCURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseXCBCURI(uriInstance, out);
}

QString formatXCBCURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("xcbc:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(XCBCUnits::format(XCBCUnits::XCBC, info.amount, false, XCBCUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openXCBCConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", XCBC_CONF_FILENAME));
    ...
    /* Open xcbc.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "XCBC.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "XCBC (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("XCBC (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "xcbc.desktop";
    return GetAutostartDir() / strprintf("xcbc-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a xcbc.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=XCBC\n";
        else
            optionFile << strprintf("Name=XCBC (%s)\n", chain);
