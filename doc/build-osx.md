...
This guide describes how to build xcbcd, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build XCBC Core from source.
...
### 4. Clone XCBC repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the XCBC Core repository to a directory.
...
It is not necessary to build wallet functionality to run `xcbcd` or  `xcbc-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/ComputationBenefactorCoin/xcbc/blob/master/doc/descriptors.md), `sqlite` is required.
...
XCBC Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/ComputationBenefactorCoin/xcbc/issues/7714).
...
You can deploy a `.dmg` containing the XCBC Core application using `make deploy`.
...
There are many ways to configure XCBC Core, here are a few common examples:
...
Run the following in your terminal to compile XCBC Core:
...
XCBC Core should now be available at `./src/xcbcd`.
If you compiled support for the GUI, it should be available at `./src/qt/xcbc-qt`.

The first time you run `xcbcd` or `xcbc-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/XCBC/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/XCBC"

touch "/Users/${USER}/Library/Application Support/XCBC/xcbc.conf"

chmod 600 "/Users/${USER}/Library/Application Support/XCBC/xcbc.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/XCBC/debug.log
```

## Other commands:

```shell
./src/xcbcd -daemon      # Starts the xcbc daemon.
./src/xcbc-cli --help    # Outputs a list of command-line options.
./src/xcbc-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/xcbc-qt -server # Starts the xcbc-qt server mode, allows xcbc-cli control
```
