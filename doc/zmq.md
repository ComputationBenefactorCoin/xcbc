...
The XCBC Core daemon can be configured to act as a trusted "border
router", implementing the xcbc wire protocol and relay, making
...
The ZeroMQ feature in XCBC Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building xcbcd:
...
For instance:
    ...
    $ xcbcd -zmqpubhashtx=tcp://127.0.0.1:28082 \
               -zmqpubhashtx=tcp://192.168.1.2:28082 \
               -zmqpubrawtx=ipc:///tmp/xcbcd.tx.raw \
...
These options can also be provided in xcbc.conf.
...
From the perspective of xcbcd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into xcbcd directly. Furthermore, no information is
...
using. XCBCd appends an up-counting sequence number to each
