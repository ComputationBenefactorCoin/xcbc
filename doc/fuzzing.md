# Fuzzing XCBC Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing XCBC Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/ComputationBenefactorCoin/xcbc
$ cd xcbc/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/ComputationBenefactorCoin/xcbc/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/ComputationBenefactorCoin/xcbc/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/ComputationBenefactorCoin/xcbc/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/ComputationBenefactorCoin/xcbc/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`xcbc-core/qa-assets`](https://github.com/ComputationBenefactorCoin/xcbc/qa-assets) repo.

To fuzz `process_message` using the [`xcbc-core/qa-assets`](https://github.com/ComputationBenefactorCoin/xcbc/qa-assets) seed corpus:

```sh
$ git clone https://github.com/ComputationBenefactorCoin/xcbc/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`xcbc-core/qa-assets`](https://github.com/ComputationBenefactorCoin/xcbc/qa-assets) repo.

Every single pull request submitted against the XCBC Core repo is automatically tested against all inputs in the [`xcbc-core/qa-assets`](https://github.com/ComputationBenefactorCoin/xcbc/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make XCBC Core more robust.
...
with certain assembly code from XCBC Core's code. See [developer notes on sanitizers](https://github.com/ComputationBenefactorCoin/xcbc/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing XCBC Core using afl++

## Quickstart guide

To quickly get started fuzzing XCBC Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing XCBC Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing XCBC Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the XCBC Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as XCBC
Core without having to write any custom fuzzing harness. The `xcbcd` server
...
```sh
$ mkdir xcbc-honggfuzz-p2p/
$ cd xcbc-honggfuzz-p2p/
$ git clone https://github.com/ComputationBenefactorCoin/xcbc
$ cd xcbc/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/xcbcd.cpp b/src/xcbcd.cpp
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/xcbcd -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
...
XCBC Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/xcbc-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=xcbc-core).
...
because XCBC is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_xcbc-core/latest).
